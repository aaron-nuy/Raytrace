...
namespace rtre {
    ...
    inline const std::vector<Vertex2> Polygone::s_Vertices = {
        Vertex2(vec2(-1, -1), vec2(0, 0)),
        Vertex2(vec2(1, -1), vec2(1, 0)),
        Vertex2(vec2(1, 1), vec2(1, 1)),
        Vertex2(vec2(-1, 1), vec2(0, 1)),
    };

    inline const std::vector<GLuint> Polygone::s_Indices = {
        0, 1, 2,
        2, 3, 0
    };
}