    ...
    public:
        Sampler2D(const char *image, GLuint unit, Senum type = rTdiffuse)
        {
            ...
            if (bytes)
            {
                ...
                {
                    case 4:
                        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, widthImg, heightImg, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                                     bytes);
                        break;
                    case 3:
                        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, widthImg, heightImg, 0, GL_RGB, GL_UNSIGNED_BYTE,
                                     bytes);
                        break;
                    case 1:
                        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, widthImg, heightImg, 0, GL_RED, GL_UNSIGNED_BYTE,
                                     bytes);
                        break;
                    default: {
    ...
    public:
        Sampler3D(const std::array<std::string, 6> &mapsides, GLuint unit, Senum type = rTdiffuse)
        {
            ...
            for (GLuint i = 0; i < 6; i++)
            {
                ...
                {
                    case 4:
                        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGBA, widthImg, heightImg, 0, GL_RGBA,
                                     GL_UNSIGNED_BYTE, bytes);
                        break;
                    case 3:
                        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGBA, widthImg, heightImg, 0, GL_RGB,
                                     GL_UNSIGNED_BYTE, bytes);
                        break;
                    case 1:
                        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGBA, widthImg, heightImg, 0, GL_RED,
                                     GL_UNSIGNED_BYTE, bytes);
                        break;
                    default: {
                        glBindTexture(GL_TEXTURE_CUBE_MAP, 0);
