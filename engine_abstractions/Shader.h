...
namespace rtre {
	...
	public:
		...
		inline void SetUniform(const char* name, const Sphere& sphere) {
			std::string uniName = std::string(name) + ".";

			glUniform1f(getUnifromID( (uniName + "radius").c_str() ), sphere.radius);
			glUniform3f(getUnifromID((uniName + "position").c_str()), sphere.position.x, sphere.position.y, sphere.position.z);
			SetUniform( (uniName + "material").c_str(), sphere.material);
		}

		inline void SetUniform(const char* name, const Box& box) {
			std::string uniName = std::string(name) + ".";

			glUniform3f(getUnifromID((uniName + "dimensions").c_str()), box.dimensions.x, box.dimensions.y, box.dimensions.z);
			glUniform3f(getUnifromID((uniName + "position").c_str()), box.position.x, box.position.y, box.position.z);

			SetUniform( (uniName + "material").c_str(), box.material );
		}

		inline void SetUniform(const char* name, const Material& mat) {
			std::string uniName = std::string(name) + ".";

			glUniform3f( getUnifromID( (uniName + "albedo").c_str() )   , mat.albedo.x, mat.albedo.y, mat.albedo.z);
			glUniform1f(getUnifromID((uniName + "roughness").c_str()), mat.roughness);
			glUniform1f(getUnifromID((uniName + "metalic").c_str()), mat.metalic);
			glUniform1f(getUnifromID((uniName + "emissive").c_str()), mat.emissive);
			glUniform1f(getUnifromID((uniName + "specular").c_str()), mat.specular);

		}

		inline void SetUniform(const char* name, const std::vector<Sphere*>& shapes) {
			std::string uniName = std::string(name) + "[";

			uint32_t counter = 0;
			for (auto shape : shapes) {
				SetUniform((uniName + std::to_string(counter) + "]").c_str(), *shape);
				++counter;
			}

		}

		inline void SetUniform(const char* name, const std::vector<Box*>& shapes) {
			std::string uniName = std::string(name) + "[";
