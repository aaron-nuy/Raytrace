...
// fixed through space
float rand() {
    unDState = unDState * 747796405u + 2891336453u;
    uint result = ((unDState >> ((unDState >> 28) + 4)) ^ unDState) * 277803737u;
...
float randPixel() {
    return fract(sin(dot(vPosition,
            vec2(12.9898, 78.233))) *
    43758.5453123 + fract(sin(scene.time / 100) + fract(rand())));
}

// random through time and space
vec3 randVecPixel() {
    return vec3(
            fract(sin(dot(vPosition.yx, vec2(12.3898, 78.233))) * 43758.5453123 + fract(scene.time) + fract(rand())),
            fract(sin(dot(vPosition, vec2(12.9498, 78.233))) * 43758.5453123 + fract(scene.time) + fract(rand())),
            fract(sin(dot(vPosition.xy, vec2(12.1828, 78.233))) * 43758.5453123 + fract(scene.time) + fract(rand()))
    );
}
...
float randDST() {
    dState = dState * 747796405u + 2891336453u;
    uint result = ((dState >> ((dState >> 28) + 4)) ^ dState) * 277803737u;
    result = (result >> 22) ^ result;
    return result / 4294967295.0;
}
...
void main() {
    ...
    if (!scene.reset)
    fragColor = mix(p, fragColor, 1 / (scene.frameCounter));
